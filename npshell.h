...
vector<Command> extractCommand(vector<string> spaceSplit){//有近來
    cout << "extractCommand" << endl;
    ...
    for(int i = 0; i < result.size(); i++){
        for(int j = 0; j < result[i].jobs.size(); j++){
            for(int k = 0; k < result[i].jobs[j].arg.size(); k++){
                cout << result[i].jobs[j].arg[k] << " ";
            }
            cout << endl;
        }
    }
...
int npshell(){//用socket傳進來都是會\r\n，用getLine會得到\r，導致execvp出錯，所以要把最後一個設成\0
    cout << "npshell" << endl;
